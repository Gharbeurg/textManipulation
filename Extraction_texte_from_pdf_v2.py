...
def is_references_header(text):
    """Détecte un vrai titre de section 'References' (ligne courte, isolée)."""
    return bool(re.match(r"^(references|bibliography)\s*$", text.strip(), re.I))
...
def clean_text(text):
    ...
    seen_abstract = False
    references_count = 0  # compteur pour éviter un faux positif isolé
    ...
    for line in lines:
        ...
        # STOP REFERENCES : uniquement si ligne courte et isolée (titre de section)
        if is_references_header(l):
            references_count += 1
            # On s'arrête seulement si c'est la 2e occurrence ou si on est loin dans le doc
            if references_count >= 1 and len(cleaned) > 20:
                break
            continue
        ...
        # FIGURE : on garde uniquement la légende (ligne suffisamment longue)
        if re.match(r"(figure|fig\.)\s*\d*[\s:.\-]", l, re.I):
            if len(l) > 30:
